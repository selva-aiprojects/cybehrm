...
async def main():
    ...
    async with engine.connect() as conn:
        ...
        wk_schemas = [s for s in schemas if "cybelinx" in s.lower()]
        print(f"Schemas containing 'cybelinx': {wk_schemas}")
        
        # 2. Organizations check
        res = await conn.execute(text("SELECT id, name, subdomain FROM public.organizations"))
        orgs = res.fetchall()
        wk_orgs = [o for o in orgs if "cybelinx" in o[1].lower() or "cybelinx" in o[2].lower()]
        print(f"Organizations containing 'cybelinx': {wk_orgs}")
        ...
        wk_emails = [e for e in emails if "cybelinx" in e.lower()]
        print(f"User emails containing 'cybelinx': {wk_emails}")
